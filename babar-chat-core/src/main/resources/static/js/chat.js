        ...
        function (msgsJson) {
            if (msgsJson != "") {
                ...
                $.each(jsonarray, function (i, msg) {
                    ...
                    if ((relation_type == 0) && (msg.ownerUid == ownerUid)) { // send from self
                        li_current.attr("id", "self-chat-li");
                        li_current.attr("avatar", 'url(/images/' + sender_avatar + ')');

                    } else if ((relation_type == 1) && (msg.ownerUid == ownerUid)) {// send from others
        ...
        function (returnContacts) {
            if (returnContacts != "") {
                ...
                $.each(jsonContacts.contactInfoList, function (index, contactInfo) {
                    ...
                    var td_button = "<td><button type='button' class='btn btn-info' data-toggle='modal' data-target='#chatModal' data-recipient_id='" + contactInfo.otherUid + "' data-recipient_name='" + contactInfo.otherName + "'>Chat</button></td>";
        ...
        function (msgsJson) {
            if (msgsJson != "") {
                ...
                $.each(jsonarray, function (i, msg) {
                    ...
                    owner_uid_avatar = msg.ownerUidAvatar;
                    other_uid_avatar = msg.otherUidAvatar;
                    if ((relation_type == 0) && (owner_uid == sender_id)) {// Send from self
                    ...
                    } else if ((relation_type == 1) && (owner_uid == sender_id)) {// Send from others
